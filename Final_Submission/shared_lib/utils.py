...
#         word = canonicalize_digits(word) # try to canonicalize numbers
#     if (wordset == None) or (word in wordset): return word
#     else: return word  #<unk> was coming up in training for word senses like 2%1:23:00::, throwing off training.
#     else: return "<unk>" # unknown token -- MODIFIED
...
def custom_batch_generator(ids, senses, batch_size, max_time):
    """Convert ids to data-matrix form."""
    # Clip to multiple of max_time for convenience
    ###TEMP COMMENT OUT
    clip_len = ((len(ids)) / batch_size) * batch_size
    input_w = ids[:clip_len]     # current word
    target_y = senses[:clip_len]  # next word
    # Reshape so we can select columns
    input_w = input_w.reshape([batch_size,-1])
    target_y = target_y.reshape([batch_size,-1])
